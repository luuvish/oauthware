 ...
 *         'authorized':    'An absolute URL to which the Service Provider will redirect the User
 *                           back when the Obtaining User Authorization step is completed',
...
OAuth2.prototype.login = function login(handler) {
  ...
  if ('function' === typeof self.login.check) {
    if (false === self.login.check.call(self, handler)) {
  ...
  if ('function' === typeof self.login.hook) {
    if (false === self.login.hook.call(self, handler)) {
      handler.redirect(self.host['base']);
      handler.session({});
      return;
    }
...
OAuth2.prototype.login.check = function check(handler) {
  return true;
};

OAuth2.prototype.login.hook = function hook(handler) {
  return true;
};
...
OAuth2.prototype.auth = function auth(handler) {
  ...
  if ('function' === typeof self.auth.check) {
    if (false === self.auth.check.call(self, handler)) {
      handler.redirect(self.host['base']);
      handler.session({});
      return;
    }
  ...
  if ('function' === typeof self.auth.hook) {
    if (false === self.auth.hook.call(self, handler)) {
      handler.redirect(self.host['base']);
      handler.session({});
      return;
    }
...
OAuth2.prototype.auth.check = function check(handler) {
  var self  = this,
      sess  = handler.session(),
      query = handler.query;
  ...
  if ('undefined' !== typeof query['error']) {
    return false;
  }

  if ('code' === self.oauth['response_type']) {
    if ('undefined' === typeof query['code']) {
      return false;
    }
    if ('authorization_code' !== self.oauth['grant_type']) {
      return false;
    }
  }

  if ('token' === self.oauth['response_type']) {
    if ('undefined' === typeof query['access_token']) {
      return false;
    }
    if ('undefined' === typeof query['token_type']) {
      return false;
    }
  }

  if ('password' === self.oauth['grant_type']) {
    if ('undefined' === typeof self.oauth['username']) {
      return false;
    }
    if ('undefined' === typeof self.oauth['password']) {
      return false;
    }
  }

  if ('undefined' !== typeof self.oauth['state']) {
    if ('undefined' === typeof query['state']) {
      return false;
    }
    if (self.oauth['state'] != query['state']) {
      return false;
    }
  }
...
OAuth2.prototype.auth.hook = function hook(handler) {
  return true;
};
...
OAuth2.prototype.api = function api(handler) {
  ...
  if ('function' === typeof self.api.check) {
    if (false === self.api.check.call(self, handler)) {
      return;
    }
  }
  ...
  if ('function' === typeof self.api.hook) {
    if (false === self.api.hook.call(self, handler)) {
      return;
    }
  }
...
OAuth2.prototype.api.check = function check(handler) {
  var self = this,
      sess = handler.session();
...
OAuth2.prototype.api.hook = function hook(handler) {
  return true;
};
