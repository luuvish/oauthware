...
OAuth2.prototype.login = function login(handle, opts) {
  var self = this;
...
OAuth2.prototype.auth = function auth(handle, opts) {
  var self = this;
  var sess = handle.session();
  ...
  for (var name in opts) {
    if ('undefined' === typeof options.body[name]) {
      options.body[name] = opts[name];
    }
  }
