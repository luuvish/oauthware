...
function Twitter(options) {
  ...
  self.fn = {
    ...
    rest: function rest(req, res, next) {
      self.rest(req, res, next);
    }
  };
  ...
  var router = connect.router(function route(app) {
    ...
    app.get(self.options.route['rest'], self.fn.rest);
  });
...
Twitter.prototype.signIn = function signIn(req, res, next) {
  ...
  oauth.post(options, function(err, data) {
    ...
    if ('true' != data['oauth_callback_confirmed']) {
      return next && next(new Error('oauth_callback_confirmed'));
    }
...
Twitter.prototype.user = function user(req, res, next) {
  var self = this;
  var sess = session(req);
...
Twitter.prototype.rest = function rest(req, res, next) {
  var self = this;
  var sess = session(req);
