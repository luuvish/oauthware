...
function request(options, callback) {
  ...
  var requestOAuth = (function prepareRequest(options) {
    ...
    return function (options, callback) {
      ...
      options = {
        ...
        path     : options.path     || url.pathname + (query ? '?' + query : ''),
        headers  : options.headers  || headers
      };

      protocol = url.protocol == 'http:' ? http : https;
      req = protocol.request(options, callback);
  ...
  return requestOAuth(responseOAuth);
  ...
  function responseOAuth(res) {
    ...
    res.on('end', function () {
      ...
      if (res.statusCode == 301 || res.statusCode == 302) {
        return requestOAuth({ url : res.headers.location }, responseOAuth);
      }
