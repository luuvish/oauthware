...
function request(options, callback) {
  ...
  function performSecureRequest(protocol, options, callback) {
    ...
    if (options['entity-body']) {
      req.write(options['entity-body']);
    }
    req.end();
  }
};
...
function makeRequest(options) {
  var opts = {
    'http-method'  : options['http-method'] || 'GET',
    'parsed-url'   : parse(options['request-url'], true),
    'content-type' : options['content-type'] || 'application/x-www-form-urlencoded',
    'entity-body'  : options['entity-body'] || {},
    'parameters'   : options['parameters'] || {}
  };

  opts['authorization'] = authorizationParameters(opts);
  opts['entity-body']   = formEncodedBody(opts['content-type'], opts['entity-body']);
  opts['query']         = requestUriQuery(opts['parsed-url'].query);

  opts.protocol = opts['parsed-url'].protocol == 'https:' ? https : http;
  opts.hostname = opts['parsed-url'].hostname;
  opts.port     = opts['parsed-url'].port;
  opts.method   = opts['http-method'].toUpperCase();
  opts.path     = opts['parsed-url'].pathname + (opts['query'] ? '?' + opts['query'] : '');
  ...
  for (var name in options.headers) {
    opts.headers[name] = options.headers[name];
  }
  opts.headers['Host']           = opts['parsed-url'].host; // MUST be hostname:port if port is exist
  opts.headers['Content-Type']   = opts['content-type'];
  opts.headers['Content-Length'] = Buffer.byteLength(opts['entity-body']);
  opts.headers['Authorization']  = authorizationHeader(opts['authorization']);

  return opts;

  function authorizationParameters(options) {
    var parameters    = options['parameters'],
        nonceLength   = parameters['oauth_nonce_length'] || 32,
        signatureKey  = {},
        authorization = {},
        name;
    ...
    for (name in parameters) {
      ...
      if ('oauth_consumer_secret' == name || 'oauth_token_secret' == name) {
        signatureKey[name] = parameters[name];
      }
      if ((/^realm$/i).test(name) || (/^oauth_/).test(name)) {
        authorization[name] = parameters[name]; 
      }
    }
    ...
    if (undefined === authorization['oauth_signature']) {
      authorization['oauth_signature'] = signature(
        signatureKey, options['http-method'], options['parsed-url'],
        authorization, options['content-type'], options['entity-body']
      );
    }
 ...
 * RFC 5849 3.4
 */
function signature(key, method, url, authorization, contentType, entityBody) {
  switch (authorization['oauth_signature_method']) {
    case 'PLAINTEXT':
      return signatureMethod['PLAINTEXT'](key);

    case 'HMAC-SHA1':
      return signatureMethod['HMAC-SHA1'](
        key,
        signatureBaseString(method, url, authorization, contentType, entityBody)
      );

    case 'RSA-SHA1':
      return signatureMethod['RSA-SHA1'](
        key,
        signatureBaseString(method, url, authorization, contentType, entityBody)
      );

    default:
      throw new Error('signature(): signature method must be PLAINTEXT or HMAC-SHA1 or RSA-SHA1');
  }
}
 ...
 */
function signatureBaseString(method, url, authorization, contentType, entityBody) {
  return [
    encodePercent(
      method.toUpperCase()
    ),
    ...
    encodePercent(
      signatureBaseStringUri(url)
    ),

    encodePercent(
      normalizeParameters(
        signatureParameters(url.query, authorization, contentType, entityBody)
      )
    )
  ].join('&');
}
 ...
 */
function signatureParameters(query, authorization, contentType, entityBody) {
  var parameters = {};

  parameters = merge(parameters, query || {});

  parameters = merge(parameters, authorization || {});

  if ('application/x-www-form-urlencoded' == contentType) {
    parameters = merge(parameters, entityBody || {});
  }
 ...
 */
signatureMethod['HMAC-SHA1'] = function (key, text) {
 ...
 */
signatureMethod['RSA-SHA1'] = function (key, text) {
  key = encodePercent(key['oauth_consumer_secret'] || '')
      + '&'
      + encodePercent(key['oauth_token_secret'] || '');
 ...
 */
signatureMethod['PLAINTEXT'] = function (key) {
  key = encodePercent(key['oauth_consumer_secret'] || '')
      + '&'
      + encodePercent(key['oauth_token_secret'] || '');
 ...
 *  (ASCII code 38).
 */
function formEncodedBody(contentType, entityBody) {

  if ('application/x-www-form-urlencoded' != contentType) return entityBody;

  if ('string' == typeof entityBody) return entityBody;

  try {
    entityBody = querystring.stringify(entityBody, '&', '=');
  } catch (e) {
    throw new Error('formEncodedBody(): cannot stringify entityBody');
  }
...
function requestUriQuery(query) {
  ...
  try {
    ...
    throw new Error('requestUriQuery(): cannot stringify query');
  }
