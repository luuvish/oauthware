...
function Twitter(options) {
  ...
  var route = options.route || '/linkedin';
  ...
  this.host = {
    ...
    'signout':       'https://api.linkedin.com/v1/account/end_session.json',
...
Twitter.prototype.api = function api(req, res, next) {
  ...
  var options = {
    'url': self.host['api'] + req.params, // + '?format=json',
//  'query': 'format=json',
    'headers': {},
    'body': {},
    'oauth': {
...
function session(req, obj) {
  ...
  if (!req.session.auth.linkedin) req.session.auth.linkedin = {};
  if ('undefined' !== typeof obj) req.session.auth.linkedin = obj;

  return req.session.auth.linkedin;
}
