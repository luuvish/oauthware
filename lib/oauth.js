...
function prepareOptions(options) {
  ...
  url.query = requestUriQuery(url.query || '');
...
function request(options, callback) {
  ...
  if (oauth) {
    var content = { type: options.headers['content-type'], body: options.body };
    if (false === oauth['oauth_signature_method'] in signature) {
      return callback && callback(
        { error: new Error('signature method must be PLAINTEXT or HMAC-SHA1 or RSA-SHA1') }
      );
    }
    options.headers['authorization'] = authorizationHeader(
      authorizationParameters(options.method, url, oauth, content)
    );
  }
